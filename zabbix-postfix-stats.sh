...
filename="ldns.$(date +'%Y%m%d')"

date="$(date +'%d')"
month="$(date +'%m')"
year="$(date +'%Y')"

# log path /logdisk/2020/08/maillog
# /logdisk/YYYY/MM/maillog/

#MAILLOG is path to the main log directory.
MAILLOG=/logdisk/$year/$month/maillog/
...
# list of logs we are extracting logs from (8)
LOGFILES = ('external-incoming' 'fmlr' 'lists' 'smtpauth' 'external-outgoing' 'ldns' 'rp' 'webmail')
